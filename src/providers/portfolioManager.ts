...
// Investment strategies configuration for StarkNet
const INVESTMENT_STRATEGIES: Record<RiskLevel, InvestmentStrategy> = {
    [RiskLevel.LOW]: {
        ...
        targetAllocation: {
            lending: {
                ...
                maxDrawdown: 5,
                minAPY: 3,
                protocols: [STARKNET_PROTOCOLS.ZKLEND, STARKNET_PROTOCOLS.NOSTRA]
            },
            stableSwap: {
                percentage: 30,
                maxDrawdown: 8,
                minAPY: 5,
                protocols: [STARKNET_PROTOCOLS.JEDISWAP]
            },
            liquidity: {
                percentage: 20,
                maxDrawdown: 10,
                minAPY: 8,
                protocols: [STARKNET_PROTOCOLS.MYSWAP]
            }
        },
...
export const portfolioManagerProvider: Provider = async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<string> => {
    try {
        ...
        if (!marketProvider || !protocolProvider || !contractProvider) {
            throw new Error("Required providers not found");
        }

        const marketData = JSON.parse(await marketProvider(runtime, message, state));
        ...
        const marketSentiment = calculateMarketSentiment(marketData);
